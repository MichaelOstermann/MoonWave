...
for await (const fileName of glob.scan("icons/")) {
    const iconData = await Bun.file(`icons/${fileName}`).json()
...
export const iconData = ${JSON.stringify(icons, null, 4)} as { name: IconName, tags: string[] }[]

export const icons = {${icons.map(icon => `\n    '${icon.name}': ${pascalCase(icon.name)}`).join(", ")}\n} as Record<IconName, LucideIcon>
`.trim())
