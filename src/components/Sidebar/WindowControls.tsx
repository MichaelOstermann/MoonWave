...
import { exit } from '@app/actions/exit'
import { minimize } from '@app/actions/minimize'
import { toggleFullscreen } from '@app/actions/toggleFullscreen'
import { toggleMaximize } from '@app/actions/toggleMaximize'
import { useEventListener } from '@react-hookz/web'
import { useState } from 'react'

export function WindowControls(): ReactNode {
    const [hasAltKey, setHasAltKey] = useState(false)

    useEventListener(window, 'keydown', (evt: KeyboardEvent) => evt.key === 'Alt' && setHasAltKey(true))
    useEventListener(window, 'keyup', (evt: KeyboardEvent) => evt.key === 'Alt' && setHasAltKey(false))

    return (
        <div className="group flex gap-x-2">
            <div
                onClick={exit}
                className="flex size-3 cursor-default items-center justify-center rounded-full border border-black/[.12] bg-[#ff544d] text-black/60 hover:bg-[#ff544d] active:bg-[#bf403a] active:text-black/60 dark:border-none"
            >
                <IconClose className="opacity-0 group-hover:opacity-100" />
            </div>
            <div
                onClick={minimize}
                className="flex size-3 cursor-default items-center justify-center rounded-full border border-black/[.12] bg-[#ffbd2e] text-black/60 hover:bg-[#ffbd2e] active:bg-[#bf9122] active:text-black/60 dark:border-none"
            >
                <IconMin className="opacity-0 group-hover:opacity-100" />
            </div>
            <div
                onClick={hasAltKey ? toggleMaximize : toggleFullscreen}
                className="flex size-3 cursor-default items-center justify-center rounded-full border border-black/[.12] bg-[#28c93f] text-black/60 hover:bg-[#28c93f] active:bg-[#1e9930] active:text-black/60 dark:border-none"
            >
                {!hasAltKey && <IconFull className="opacity-0 group-hover:opacity-100" />}
                {hasAltKey && <IconPlus className="opacity-0 group-hover:opacity-100" />}
            </div>
        </div>
    )
}
...
function IconMin(props: ComponentProps<'svg'>): ReactNode {
    return (
        <svg
            width="8"
            height="8"
...
function IconFull(props: ComponentProps<'svg'>): ReactNode {
    return (
        <svg
            width="6"
            height="6"
...
function IconPlus(props: ComponentProps<'svg'>): ReactNode {
    return (
        <svg
            width="8"
            height="8"
